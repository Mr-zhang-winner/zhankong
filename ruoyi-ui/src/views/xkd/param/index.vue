<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="参数类型：SYSTEM/DEVICE/SATELLITE/ALARM/STORAGE/TASK" prop="paramType">
        <el-select v-model="queryParams.paramType" placeholder="请选择参数类型：SYSTEM/DEVICE/SATELLITE/ALARM/STORAGE/TASK" clearable>
          <el-option
            v-for="dict in dict.type.xkd_param_type"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="paramList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="参数类型：SYSTEM/DEVICE/SATELLITE/ALARM/STORAGE/TASK" align="center" prop="paramType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.xkd_param_type" :value="scope.row.paramType"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="参数类型：SYSTEM/DEVICE/SATELLITE/ALARM/STORAGE/TASK" prop="paramType">
              <el-select v-model="form.paramType" placeholder="请选择参数类型：SYSTEM/DEVICE/SATELLITE/ALARM/STORAGE/TASK">
                <el-option
                  v-for="dict in dict.type.xkd_param_type"
                  :key="dict.value"
                  :label="dict.label"
...
<script>
import { listParam, getParam, delParam, addParam, updateParam } from "@/api/xkd/param"

export default {
  name: "Param",
  dicts: ['xkd_param_type', 'sys_normal_disable'],
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        ...
        paramType: [
          { required: true, message: "参数类型：SYSTEM/DEVICE/SATELLITE/ALARM/STORAGE/TASK不能为空", trigger: "change" }
        ],
      }
    }
  },
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getParam(paramId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除系统参数编号为"' + paramIds + '"的数据项？').then(function() {
        return delParam(paramIds)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('xkd/param/export', {
        ...this.queryParams
