...
public class AntTxFreqParam {
    ...
    /** 发射频率（32位无符号整数，单位：Hz） */
    private U32Union antTxFreq;

    /** 默认构造函数 */
    public AntTxFreqParam() {
        this.antTxFreq = new U32Union();
        this.len = 4;  // 固定长度
    }
     ...
     * 带初始值的构造函数
     * @param key 参数键值
     * @param freq 发射频率（单位：Hz）
     */
    public AntTxFreqParam(int key, long freq) {
        this.key = key;
